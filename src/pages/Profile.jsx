...
export default function Profile() {
  ...
  useEffect(() => {
    async function fetchUserDetails() {
      try {
        const fetchedUser = await fetchUser(user); //Fetch user info
    ...
    <div>
      ...
      {posts.length > 0 ? (
        posts.map((post) => (
          <div
            key={post._id}
            className='container mt-5 border shadow rounded'
          >
            <h3>{post.title}</h3>
            ...
            <p>{post.location}</p>
            <p>
              {post.willDeliver ? 'Delivery Available' : 'Pickup Available'}
            </p>
